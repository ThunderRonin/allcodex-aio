...
5. **Portal E2E requires full stack**: allcodex-portal has vitest unit tests (`lib/*.test.ts`, 258 tests) and a Playwright E2E suite (`tests/*.spec.ts`, ~150 specs across chromium/firefox/webkit). `bun run check` excludes Playwright intentionally — E2E needs AllKnower (:3001) and AllCodex Core (:8080) live. Run `bun run test:e2e` from `allcodex-portal/` when the full stack is up; Playwright auto-starts the dev server. A separate `integration` Playwright project (`tests/integration/*.spec.ts`, 4 specs) runs against real LLMs — requires `TEST_OPENROUTER_API_KEY`, `TEST_ALLCODEX_ETAPI_TOKEN`, and `TEST_ALLKNOWER_URL` in `.env.test`. Global setup auto-provisions a test account and connects AllCodex Core. Run with `npx playwright test --project=integration`.
